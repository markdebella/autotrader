 ...
 * NEVER places orders and the LLM is NEVER in the page (see ROADMAP.md): ideas are
 * generated by the backend service (Google Gemini, or a deterministic rules engine) and
 * saved to the user's Drive (recommendations.json) by the browser. This module defines the
 * schema, seeds samples so the feed is explorable, and turns an approved recommendation
 * into a natural-language command to run in Claude Code (the Alpaca MCP execution tool).
 *
...
const Recs = (() => {
  ...
  /** A fresh sample document so the feed is explorable before real ideas are generated. */
  function sampleDoc() {
